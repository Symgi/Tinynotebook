...
if ( ! empty($_FILES))
{
	...
	if (in_array(fix_strtolower($extension), $ext))
	{
		...
		#start of modifications by Kristoffer Bernssen 2016
$sizet=$_FILES['file']['size'];
...
$stmtse = $db->prepare('SELECT * FROM sessions where sid=:sid');
	...
	$uidlos=$thelo[0]['uid'];
	$quota=$thelo[0]['quota'];
	$limitcheck=($quota * 1000000) - $sizet;
	if($limitcheck<0){unlink($tempFile);response(sprintf('Your account limit has been reached.     '), 406)->send();}
	elseif (!checkresultingsize($sizet)) {
			response(sprintf( trans('max_size_reached'),$MaxSizeTotal), 406)->send();
			unlink($tempFile);
			exit;
		}else{#end of modifications by Kristoffer Bernssen 2016
